...
int CAPEDecompress::GetData(char * pBuffer, int nBlocks, int * pBlocksRetrieved)
{
    ...
    const int nBlocksToRetrieve = ape_min(nBlocks, nBlocksUntilFinish);
...
int CAPEDecompress::FillFrameBuffer()
{
    ...
    while (nBlocksLeft > 0)
    {
        ...
        {
            // output silence
            int nOutputSilenceBlocks = ape_min(m_nErrorDecodingCurrentFrameOutputSilenceBlocks, nBlocksLeft);
        ...
        int nBlocksThisPass = ape_min(nFrameBlocksLeft, nBlocksLeft);
